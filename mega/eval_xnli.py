...
def load_xnli_dataset(lang: str) -> Union[datasets.Dataset, datasets.DatasetDict]:
    """
    Args:
        lang (str): Language for which xnli dataset is to be loaded

    Returns:
        Union[datasets.Dataset, datasets.DatasetDict]: huggingface dataset object
    """
...
def choose_few_shot_examples(
    train_dataset: datasets.Dataset, few_shot_size: int, selection_criteria: int
...
def evaluate_model(
    ...
    Args:
        train_dataset (datasets.Dataset): _description_
        test_dataset (datasets.Dataset): _description_
...
def main():
    ...
    train_dataset = load_xnli_dataset(args.pivot_lang)["train"]
    test_dataset = load_xnli_dataset(args.tgt_lang)["validation"]
    ...
    out_dir = f"{args.save_dir}/xnli/{args.model}/{args.tgt_lang}/PivotLang_{args.pivot_lang}_PromptName_{args.tgt_prompt_name}_FewShotK_{args.few_shot_k}"
