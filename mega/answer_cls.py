...
def get_few_shot_samples(train_dataset, few_shot_size, selection_criteria, sent_tokenizer, qrpc = False):
    if qrpc:
        qrpc_dataset = load_dataset("glue", "qnli")["train"]
        fs_examples = choose_few_shot_examples(qrpc_dataset, few_shot_size, selection_criteria)
        ...
        fs_examples = choose_few_shot_examples(train_dataset, few_shot_size, selection_criteria)
        fs_prompt = []
        for example in fs_examples:
            ...
            for sent in context_sents:
                if answer in sent:
                    prompt = PROMPT_TEMPLATE.replace("{question_str}", question).replace("{context_str}", sent)
                    prompt += "Yes\n\n"
...
def get_line_with_answer(example, sent_tokenizer, model, fs_examples = "", **model_params):
    question = example["question"]
    answer = example["answers"]["text"][0]
    context = example["context"]
    context_sents = sent_tokenizer(context)
    ...
    for sent in context_sents:
        prompt = fs_examples + PROMPT_TEMPLATE.replace("{question_str}", question).replace("{context_str}", sent)
        try:
    ...
    return predicted_context_with_answer, int(predicted_context_with_answer == context_with_answer)
...
def main(sys_args):
    ...
    fs_examples = get_few_shot_samples(train_dataset, args.few_shot_k // 2, args.few_shot_selection, tokenizer)
    # Zero-Shot evaluation
    matches = [get_line_with_answer(example, tokenizer, model = "DaVinci003", fs_examples = fs_examples)[1] for example in tqdm(test_dataset)]
    acc = (np.mean(matches))
    print(acc)
