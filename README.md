...
## Transparency Note
MEGA includes code to benchmark Large Language Models on several multilingual datasets. However, we cover only around 70 languages in all, with most datasets having around 8 languages. Hence, it is not possible to get a complete picture of the model's performance on all languages supported by the model. Although we compare the evaluation results of GPT-3.5 and GPT-4 with BLOOMZ and SOTA models, we could not evaluate other closed models such as PaLM, which also contains training data in many languages. A limitation of our study is that we do not evaluate on all the multilingual datasets that are available, and we plan to scale up our evaluation in future versions of the study to cover more tasks and languages. Multilingual datasets available today do not cover many typologically diverse and under-resourced languages, which is a fundamental limitation of trying to scale up multilingual evaluation today. For example, there is very little representation from African languages, Indigenous languages of the Americas etc. in any of the evaluation benchmarks available today. We restrict ourselves to the performance metrics and to some extent gender bias dimension of evaluation for this study - however, we plan to include evaluation of calibration, toxicity, bias, robustness, etc. in future work. Prompt-based models are very sensitive to prompting variations. Although we standardize prompts to the extent possible in our study, it is possible that results may vary with changes in prompts, leading to different conclusions than the ones we draw. Our evaluation is based on standard multilingual benchmarks that are created by humans or with human involvement. However, we recommend conducting an evaluation study involving native speakers of a language and the target user community before releasing an application.
