...
do
    ...
    do
        k=4
        echo "Running for language $lang and prompt ${prompt_name} and k $k"
        python -m mega.eval_xstory_cloze -d xstory_cloze -e gpt4v2 -p en -t $lang --model "gpt-35-turbo" --tgt_prompt_name "${prompt_name}" --temperature 0 --log_wandb -k $k --timeout 30 --chat-prompt
    done
done
...
do
    ...
    do
        k=4
        echo "Running for language $lang and prompt ${prompt_name} and k $k"
        python -m mega.eval_xstory_cloze -d xstory_cloze -e gpt4v3 -p en -t $lang --model "text-davinci-003" --tgt_prompt_name "${prompt_name}" --temperature 0 --log_wandb -k $k --timeout 30
    done
done
